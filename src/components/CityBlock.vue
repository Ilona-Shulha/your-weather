<template>
  <div class="card">
    <div v-if="!isFavoriteCard && !isUserCityCard" class="city-block-data">
      ...
      <button v-if="cityIndex > 0" @click="() => deletionConfirmation = true" class="basic-button">Delete city block</button>
    </div>
    ...
    <div class="city-data">
      ...
      <button v-if="canAddToFavorite && !isUserCityCard" @click="handleAddingFavoriteCity">
        <img class="star" :src="require('@/assets/imgs/stroke_star.svg')">
      </button>
      <button v-if="isFavoriteCard || (props.cityData.id && !canAddToFavorite && !isUserCityCard)" @click="handleDeletingFavoriteCity">
        <img class="star" :src="require('@/assets/imgs/fill_star.svg')">
      </button>
    </div>
      <button v-if="props.cityData.id" :disabled="forecastType==='hourly'" class="toggle-btn basic-button" @click="() => forecastType='hourly'">Today</button>
      <button v-if="props.cityData.id" :disabled="forecastType==='daily'" class="basic-button toggle-btn toggle-btn_right" @click="() => forecastType='daily'">5 days</button>
      <BasicLoader v-if="cityData?.id && !cityData?.weather" class="loader"/>
    <WeatherInfoBlock v-if="cityData?.weather" :forecastType="forecastType" :weatherDataList="cityData?.weather?.list" />
    <ModalWindow :show="deletionConfirmation">
      <p>Are you sure you want to delete this weather block?</p>
      <button @click="()=>deletionConfirmation = false" class="basic-button">Cancel</button>
      <button @click="deleteNewCityBlock" class="basic-button basic-button-right">Ok</button>
    </ModalWindow>
    <ModalWindow :show="cantAddToFavorite">
      <p>To add a city, delete some city from your favorites, because there are a maximum of 5 favorite cities</p>
      <button @click="()=>cantAddToFavorite = false" class="basic-button" >Ok</button>
    </ModalWindow>
    </template>
  </div>
</template>
<script setup>
  ...
  const canAddToFavorite = computed(() => {
    return props.cityData.id && !weatherStore.favoriteCities.some(city => city.id === props.cityData.id)
  })
